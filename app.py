...
if os.path.exists("Allianz.svg"):  # Verificar si el archivo está disponible
...
if 1 <= len(seleccion_etfs) <= 3:
    ...
    if st.button('Simular inversión y comparar ETFs'):
        ...
        for idx, etf_info in enumerate(etf_info_list):
            ...
            elif 'Adj Close' not in datos.columns:  # Verificar si la columna 'Adj Close' existe
                st.error(f"El ETF {etf_info['nombre']} no tiene datos de 'Adj Close' disponibles.")
            else:
